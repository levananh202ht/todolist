...
class App extends React.Component {
  ...
  handleEdit = (event) =>{
    // const {todolist} = this.state;
    // debugger
    // const newEdit = todolist.findIndex(item =>  item.id === event.id)
    // todolist[newEdit].isCompleted = !todolist.isCompleted;
    if (this.headerRef.current) {
      this.headerRef.current.focusInput();
    }
    const { todolist } = this.state;
    const newTodolist = todolist.filter(item =>  item.id !== event.id)
    const newEdit = todolist.filter(item =>  item.id === event.id)
    console.log(newEdit);
    debugger
    this.setState({
