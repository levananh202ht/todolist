...
function App() {
  ...
  const addItem = (item) => {  
    const newTodoList = [item, ...newTodoList]
    newTodoList = {
      isCompleted: todolist[item].isCompleted ===  newFilter.Active ?  newFilter.Completed :  newFilter.Active,
    }
  ...
  const renderFilter = () => {
    ...
    todolist.filter(item => filterTodo === newFilter.All || filterTodo  === item.isCompleted)
  };
