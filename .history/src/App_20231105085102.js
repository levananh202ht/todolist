...
class App extends React.Component {
  ...
  handleActiveClick = () => { 
    const {todolist} = this.state; 
    const todoListFiltered = todolist.map((item) => item.isCompleted === newFilter.ACTIVCE);
    this.setState({
      todolist: todoListFiltered
      });
  };
  handleCompletedClick = () => {
    const {todolist} = this.state;  
    const todoListFiltered = todolist.map((item) => item.isCompleted === newFilter.Completed);
    this.setState({
      todolist: todoListFiltered
      });
  };
