...
class App extends React.Component {
  ...
  handleAllClick = () => {
    const {todolist} = this.state;
    const todoListFiltered = todolist.filter((item) => ({...item}));
    this.setState({
      //todolist: todoListFiltered,
      CompletedFooter: todoListFiltered
    })
  }
  handleActiveClick = () => { 
    const {todolist} = this.state; 
    const todoListFiltered = todolist.filter((item) => item.isCompleted === newFilter.Active);
    debugger
    this.setState({
     // todolist: todoListFiltered
      CompletedFooter: todoListFiltered
