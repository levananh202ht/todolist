...
class App extends React.Component {
  ...
  handleEdit = (name,id,event)=>{
    console.log("Đã click vào phần tử:", event.target);
    const {todolist} = this.state;
    todolist: todolist.map(item => item.name === name)
    if (this.headerRef.current) {

      this.headerRef.current.focusInput(event.target.name);
    }
