...
class App extends React.Component {
  ...
  handleAllClick = () => {
    const {todolist} = this.state;
    const todoListFiltered = todolist.map((item) => (item.isCompleted = newFilter.All));
    this.setState({
  ...
  handleActiveClick = () => { 
    ...
    this.setState({
      todoListFiltered: todolist.map(
        (item) => item.isCompleted === newFilter.Active
      ),
      });
  };
  handleCompletedClick = () => {
    ...
    this.setState({
      todoListFiltered: todolist.filter(
        (item) => item.isCompleted === newFilter.Completed
      ),
      });
  };
  deleteItem = (id) => {
