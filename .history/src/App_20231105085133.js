...
class App extends React.Component {
  ...
  handleAllClick = () => {
    const {todolist} = this.state;
    const todoListFiltered = todolist.find((item) => ({...item}));
    this.setState({
  ...
  handleActiveClick = () => { 
    const {todolist} = this.state; 
    const todoListFiltered = todolist.find((item) => item.isCompleted === newFilter.ACTIVCE);
    this.setState({
      todolist: todoListFiltered
      });
  };
  handleCompletedClick = () => {
    const {todolist} = this.state;  
    const todoListFiltered = todolist.find((item) => item.isCompleted === newFilter.Completed);
    this.setState({
      todolist: todoListFiltered
      });
  };
